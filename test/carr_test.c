...
int carr_test(const int argc, char* argv[])
{
   ...
   CTEST_UNIT("carr_split")
   {
      ...
      int i = 0;
      for_carr(lines) {
         EQS( (char*)carr_getsi(lines) , expect_lines[i++], "Verify car_getsi");
      }
...
/*
      EQ(carr_len(lines), 8, "Verify number of lines after carr_make_space")
      count = 0;
      for_carr (lines) {
         // EQ(carr_i(lines), count, "Verify iterator")
         char* line;
